...
func main() {
	if err := config.Load(configPath); err != nil {
		logger.Error(context.Background(), "failed to load user config: %v", zap.Error(err))
		return
	}
	...
	if err != nil {
		logger.Error(appCtx, "❌ Не удалось создать user приложение", zap.Error(err))
		return
	}

	err = a.Run(appCtx)
	if err != nil {
		logger.Error(appCtx, "❌ Ошибка при работе user приложения", zap.Error(err))
		return
	}
}
